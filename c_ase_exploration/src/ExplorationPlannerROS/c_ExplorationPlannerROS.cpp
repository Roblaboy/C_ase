    ...
    while(ros::ok() && explore_server_.isActive())
    {
        ...
        if(1)
        {
            ros::ServiceClient laser_nav = 
                    nh_.serviceClient<ase_exploration::laser_nav_srv>("select_laser_nav_goal");

            //service call to select_laser_nav_goal
            ase_exploration::laser_nav_srv laser_srv;

            //give current pose based on robot's frame_id to laser navigation
            laser_srv.request.base_frame_id = params_.base_frame_id_;
            laser_srv.request.map_frame_id = params_.map_frame_id_;
            ...
            if(laser_nav.call(laser_srv))
            {
                ...
                for(int i=0;i<laser_srv.response.x_map.size();++i)
                {
                    x_map.push_back(laser_srv.response.x_map.at(i));
                    y_map.push_back(laser_srv.response.y_map.at(i));
                }
            }
            else
            {
