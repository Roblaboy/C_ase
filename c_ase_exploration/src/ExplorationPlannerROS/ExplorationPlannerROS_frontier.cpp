...
void ExplorationPlannerROS::executeCb(const ase_exploration::ExploreGoalConstPtr &goal)
{
 ...
 //*       //while ( (planner_iteration < params_.num_kernels) && explore_server_.isActive() )//开始迭代
  ...
  //*      if (!possibly_stuck_)
		...
		ROS_INFO("X:%f Y:%f",goal_pose.pose.position.x,goal_pose.pose.position.y);
            }
            else
            {
...
void ExplorationPlannerROS::feedbackMovingCb(const move_base_msgs::MoveBaseFeedbackConstPtr& feedback)//如果不是边界目标任务执行超时，则取消任务，如果是边界目标，距离小于阈值，也取消任务
{
