...
float laser_angle_step_deg=0.3515625;
float max_dist=3.5;
...
class laser_nav
{
    ...
    int seq_point_numbers(double dis);//对应最远处允许的第一类边界的最小长度的激光点数
    std::vector<float> x,y;
    float laser_data[512];  
    geometry_msgs::PointStamped goal_pose_base;
    geometry_msgs::PointStamped goal_pose_map;
    void scancallback(const sensor_msgs::LaserScan::ConstPtr& scan);
};

int laser_nav::seq_point_numbers(double dis)
{
    int num=0;
    num=Robot_max_dia/dis/Pi*180/laser_angle_step_deg+1;
    return num;
    ROS_INFO("num=%d",num);
}
               ...
               laser_nav_::laser_nav_srv::Response &res)
{
    ...
    //****************一类目标点*********************//
        if(laser_data[i]>=max_dist) 
        {
            ...
            for(int j=i;j<ranges;++j)
            {
                if((laser_data[j]>=max_dist) &&j!=(ranges-1))
                {
                    ++disc;
                }
                else if(disc > seq_point_numbers(laser_data[i-1])) 
                {
                    end_n=j;//遇到第一个不连续点，并且在这之前已经有连续的14个最远点，标记此弧线末端激光位置
                    ROS_INFO("i:%d j:%d",i,j);
                    theta=ori_cal(i,j);
                    x.push_back((laser_data[i]-0.8)*sin(theta));
                    y.push_back(-(laser_data[i]-0.8)*cos(theta));                   
                    disc=0;
                    i=j;
                    break;//扫完一段边界，跳出这层for循环
                }
                else {disc=0;break;}//遇到第一个不连续点，并且在这之前没有连续的14个最远点，跳出这层for循环，继续扫描下一段
            }
        }
        //****************二类目标点*********************//
        if(fabs(laser_data[i]-laser_data[i-1])>=1.5)
        {
            ...
            {
                if(fabs(laser_data[j]-laser_data[i-1])>=1.5)
                {
                    disc++;
                }
                else if(disc > seq_point_numbers((laser_data[i]+laser_data[i-1])/2-fabs(laser_data[i]-laser_data[i-1])/2)) //取小的那边长度
                {
                    disc = 0;
                    if((laser_data[i]<max_dist)&&(laser_data[i-1]<max_dist))
                    {
                        if(laser_data[i-1]-laser_data[i]>0)
                        {
                            ...
                            x.push_back(0.5*(laser_data[i-1]+laser_data[i])*sin(theta));
                            y.push_back(-0.5*(laser_data[i-1]+laser_data[i])*cos(theta));
                        }
                        else
                        {
